...
st.info("Workflow following the official flowchart. All techniques powered by XGBoost AI.")
...
# --- STEP 3: CHOOSE FORECAST TECHNIQUE (Strict Flowchart Items) ---
...
# Capture specific inputs required by the flowchart
tech_inputs = {}
if technique == "Weightage Average":
    w_mode = st.radio("Weight Mode", ["Automated (Even)", "Manual Entry"], horizontal=True)
    if w_mode == "Manual Entry":
        w_text = st.text_input("Enter weights (e.g. 0.2, 0.3, 0.5)", "0.2, 0.3, 0.5")
        tech_inputs['weights'] = [float(x.strip()) for x in w_text.split(',')]
    else:
        tech_inputs['window'] = st.number_input("Lookback Window (Number of periods)", 2, 12, 3)

elif technique == "Moving Average":
    tech_inputs['window'] = st.number_input("Lookback Window (Number of periods)", 2, 24, 3)

elif technique == "Ramp Up Evenly":
    tech_inputs['ramp_factor'] = st.number_input("Interval Ramp up Factor (Multiplier)", 1.0, 5.0, 1.05, 0.01)
...
# --- INTERNAL XGBOOST ENGINE ---
def run_ai_logic(data, horizon_days, interval_type, tech, params):
    df = data.copy()
    ...
    # Determine sample weights for training based on technique
    sample_weights = np.ones(len(df))
    if tech == "Weightage Average" and 'weights' in params:
        w_list = params['weights']
        # Apply manual weights to the most recent records
        for i, weight in enumerate(reversed(w_list)):
            if (len(df) - 1 - i) >= 0:
                sample_weights[len(df) - 1 - i] = weight
    
    # If Moving Average, only train on the last X records
    if tech == "Moving Average":
        df = df.tail(params['window'])
        sample_weights = sample_weights[-len(df):]

    # Initialize XGBoost (Fixed optimized parameters)
    ...
    # Apply Ramp Up Factor if selected
...
if uploaded_file:
    try:
        # Load and Transform (Wide format handling)
        raw = pd.read_csv(uploaded_file) if uploaded_file.name.endswith('.csv') else pd.read_excel(uploaded_file)
        raw.columns = raw.columns.astype(str).str.strip()
        id_col = raw.columns[0]
        
        # Detect valid date columns
        ...
        if st.button("🚀 Run Analysis", use_container_width=True):
            with st.spinner('XGBoost is analyzing trends...'):
                ...
                res_df = pd.DataFrame({"Forecast Date": f_dates.strftime(date_fmt), "Predicted Quantity": np.round(f_values, 1)})
