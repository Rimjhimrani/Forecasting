...
# --- 1. UI SETTINGS & MINIMALIST WHITE CSS ---
st.set_page_config(page_title="AI Precision Forecast", layout="centered", page_icon="📊")
...
# --- HEADER ---
st.title("📊 AI Precision Forecast")
st.markdown("Follow the steps below to generate your supply chain forecast.")

# --- STEP 1: SCOPE ---
st.markdown('<div class="step-box"><div class="step-label">Step 1</div><div class="step-title">Select Forecasting Scope</div>', unsafe_allow_html=True)
main_choice = st.radio("Primary Path", ["Aggregate Wise", "Product Wise"], horizontal=True)
sub_choice = None
if main_choice == "Product Wise":
    sub_choice = st.radio("Specify Level", ["Model Wise", "Part No Wise"], horizontal=True)
st.markdown('</div>', unsafe_allow_html=True)

# --- STEP 2: TIMELINE ---
st.markdown('<div class="step-box"><div class="step-label">Step 2</div><div class="step-title">Timeline Configuration</div>', unsafe_allow_html=True)
...
# --- STEP 3: STRATEGY ---
st.markdown('<div class="step-box"><div class="step-label">Step 3</div><div class="step-title">Select Strategy & AI Technique</div>', unsafe_allow_html=True)
...
# --- STEP 4: UPLOAD ---
st.markdown('<div class="step-box"><div class="step-label">Step 4</div><div class="step-title">Data Ingestion</div>', unsafe_allow_html=True)
...
if uploaded_file:
    try:
        ...
        if main_choice == "Aggregate Wise":
            ...
            selected = st.selectbox(f"Select Target {sub_choice if sub_choice else ''}", df_long[id_col].unique())
        ...
        if st.session_state.get('run_analysis', False):
            ...
            st.subheader("🔄 Live Adjustment")
            c1, c2 = st.columns(2)
            with c1: dynamic_val = st.number_input("Qty", min_value=1, value=15)
            with c2: dynamic_unit = st.selectbox("Unit", ["Days", "Weeks", "Months", "Original Selection"])
            ...
            for i, res in enumerate(ai_residuals, 1):
                ...
                excel_calc_col.append(round(base, 2)); predicted_calc_col.append(round(max(base + res, 0), 2))

            # --- PLOTTING ---
            st.subheader(f"📈 Trend Forecast: {item_name}")
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=target_df['Date'], y=target_df['qty'], name="Traded", line=dict(color="#2563eb", width=2)))
            
            f_dates_conn = [last_date] + list(future_dates)
            fig.add_trace(go.Scatter(x=f_dates_conn, y=[last_qty] + excel_calc_col, name="Baseline", line=dict(color="#94a3b8", dash='dot')))
            fig.add_trace(go.Scatter(x=f_dates_conn, y=[last_qty] + predicted_calc_col, name="AI Prediction", line=dict(color="#10b981", width=3)))
            ...
            fig_wig.update_layout(template="plotly_white", height=200, margin=dict(l=0,r=0,t=0,b=0))
            st.plotly_chart(fig_wig, use_container_width=True)

            # --- DATA TABLE ---
            st.subheader("📋 Results")
            res_df = pd.DataFrame({"Date": future_dates.strftime('%d-%m-%Y'), "AI Prediction": predicted_calc_col, "Excel Baseline": excel_calc_col})
            st.dataframe(res_df, use_container_width=True, hide_index=True)
            
            output = io.BytesIO()
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                res_df.to_excel(writer, index=False)
            st.download_button("📥 Download Excel Report", output.getvalue(), f"AI_Forecast_{item_name}.xlsx")
            st.markdown('</div>', unsafe_allow_html=True)
