...
<style>
    ...
    div.stButton > button { width: 100%; background-color: #111827 !important; color: #FFFFFF !important; border: none !important; padding: 18px !important; font-size: 1.1rem !important; font-weight: 600 !important; border-radius: 10px !important; transition: 0.3s; }
    div.stButton > button:hover { background-color: #4F46E5 !important; transform: translateY(-2px); }
    .insight-card { background-color: #F8FAFC; border: 1px solid #E2E8F0; padding: 30px; border-radius: 20px; margin-top: 40px; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1); }
</style>
...
def get_display_labels(dates, interval, is_forecast=False):
    ...
    - Forecast Weekly/Daily/Hourly: Sequential (Week 1, Week 2...)
    - Forecast Monthly/Quarterly/Yearly: Calendar (Jan 25, Q1-25...)
    - History: Always Calendar
    """
    if is_forecast:
        if interval == "Weekly": return [f"Week {i+1}" for i in range(len(dates))]
        if interval == "Daily": return [f"Day {i+1}" for i in range(len(dates))]
        if interval == "Hourly": return [f"Hr {i+1}" for i in range(len(dates))]
    
    # Calendar Formatting
    if interval == "Monthly": return dates.strftime('%b %y')
    if interval == "Quarterly": return [f"Q{d.quarter}-{str(d.year)[2:]}" for d in dates]
    if interval == "Year": return [f"Year {str(d.year)[2:]}" for d in dates]
    if interval == "Daily": return dates.strftime('%d %b')
    if interval == "Weekly": return dates.strftime('Wk %U-%y')
...
# --- 4. UI HEADER ---
st.markdown('<div style="text-align: center; margin-bottom: 60px;">'
            '<h1 style="font-size: 3.5rem; font-weight: 800; letter-spacing: -2px; color: #4F46E5;">Agilo<span style="color:#111827;">Forecast</span></h1>'
            '<p style="color: #6B7280; font-size: 1.2rem;">Enterprise AI Supply Chain Intelligence</p>'
...
with c1:
    interval = st.selectbox("Interval", options=["Hourly", "Daily", "Weekly", "Monthly", "Quarterly", "Year"], index=2)
with c2:
    horizon_unit = st.selectbox("Horizon Unit", ["Day(s)", "Week(s)", "Month(s)", "Quarter(s)", "Year(s)"], index=1)
with c3:
...
with h2:
    ...
    else: st.info("Analyzing full historical dataset.")
with h3:
    if hist_scope == "Custom Lookback":
        hist_val = st.number_input("Value", min_value=1, value=6, key="hist_val")
...
with c5:
    ...
    if technique == "Weightage Average":
        ...
        tech_params['n'] = st.number_input(f"Window Size", min_value=1, max_value=100, value=3)
    elif technique == "Ramp Up Evenly":
        st.info("Algorithm will detect linear ramp from historical baseline.")
...
if uploaded_file:
    try:
        ...
        # Identify date columns (skipping first 4 metadata columns)
        ...
        if main_choice == "Aggregate Wise":
            ...
            if sub_choice == "Model Wise":
                ...
                p_sel = st.selectbox("Part Number", df_long[df_long[model_col] == m_sel][part_no_col].unique())
        ...
        if st.button("EXECUTE AI FORECAST"):
            if len(target_df) < 2:
                st.error("Insufficient historical data for this selection/lookback.")
            else:
                ...
                # 2. AI Residual Correction (XGBoost)
                ...
                fig = go.Figure()
                # Historical Trace
                fig.add_trace(go.Scatter(x=hist_labels, y=target_df['qty'], name="Historical Demand", line=dict(color="#1a8cff", width=2)))
                # Forecast Trace (Joined)
                conn_x = [hist_labels.iloc[-1]] + list(future_labels)
                ...
                # 5. Demand Schedule Table
                st.markdown(f"#### 📋 {interval} Demand Schedule: {item_name}")
                res_df = pd.DataFrame({
                    "Period": future_labels,
                    "AI Predicted Demand": ai_vals,
                    "Baseline Demand": [round(b, 2) for b in baseline_vals]
                })
